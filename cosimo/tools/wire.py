...
Canonical home since v3. Generation (`dataset/pipelines/v3`), training
(`jobs/fine-tune/cosimo_ft/tools.py` re-exports this module) and serving all
import the wire contract through `cosimo.tools`, so a training target, a corpus
tool-call and a served prompt can never be three slightly different strings.
