...
def v2_cr_counterparty(rng, seq):
    ...
    tr = (_assume(["CVA = E × (1 − R) × PD × exp(−rT)"]) +
          f"CVA = {fmt(exp)} × (1 − {pct(lgd_cp)}) × {pct(pd_cp)} × exp(−{pct(r)} × {t})\n"
          f"CVA = {fmt(exp)} × {pct(1 - lgd_cp)} × {pct(pd_cp)} × {math.exp(-r * t):.4f}\n"
          f"= {fmt(cva)}\n"
...
def v2_cr_securitization(rng, seq):
    ...
    tr = (_assume(["Waterfall: interest first, then principal down the seniority chain"]) +
          f"Tranche sizes: A = {fmt(a_princ)}, B = {fmt(b_princ)}, C = {fmt(c_princ)}\n"
          f"Coupon rates: A = {pct(a_cpn)}, B = {pct(b_cpn)}, C = {pct(c_cpn)}\n"
          f"Pool available = {fmt(avail_pool)}\n\n"
          "Step 1. Compute interest per tranche:\n"
          f"  A_int = {fmt(a_princ)} × {pct(a_cpn)} = {fmt(a_int)}\n"
          f"  B_int = {fmt(b_princ)} × {pct(b_cpn)} = {fmt(b_int)}\n"
          f"  C_int = {fmt(c_princ)} × {pct(c_cpn)} = {fmt(c_int)}\n"
          f"  Total interest = {fmt(total_int)}\n\n"
          f"Step 2. Available for principal = {fmt(avail_pool)} − {fmt(total_int)} = {fmt(avail_principal)}\n\n"
          f"Step 3. Waterfall: A gets {fmt(a_paid)}, B gets {fmt(b_paid)}, C gets {fmt(c_paid)}\n\n"
          f"Step 4. The portfolio weighted-yield = {pct(wt_yld)}.\n"
...
def v2_op_loss_dist(rng, seq):
    ...
    tr = (_assume(["Severity ~ lognormal(μ,σ); Loss L = Σ Xi ≈ Normal(λ·m, λ·s²)"]) +
          f"Frequency λ = {lam:.1f} events/yr\n"
          f"Severity: μ = {mu_s:.2f} (→ mean = {fmt(math.exp(mu_s + 0.5 * sigma_s**2))}), "
          f"σ = {sigma_s:.2f} (→ std = {fmt(std_sev)})\n\n"
          f"Step 1. Expected loss = λ × E[X] = {lam:.1f} × {fmt(math.exp(mu_s + 0.5 * sigma_s**2))} = {fmt(exp_loss)}\n"
          f"Step 2. Std loss = √λ × σ = {math.sqrt(lam):.2f} × {fmt(std_sev)} = {fmt(std_loss)}\n"
          f"Step 3. 99.5th-percentile ≈ {fmt(exp_loss)} + 2.326 × {fmt(std_loss)} = {fmt(var995)}\n"
          f"Step 4. Economic capital = {fmt(var995)} − {fmt(exp_loss)} = {fmt(eco_cap)}\n\n"
    ...
    flaw = {"answer": f"{fmt(var995)}",
            ...
            "reasoning_trace": (_assume(["EC = 99.5th %ile"]) +
                                f"Step 1. Quoting the 99.5% VaR = {fmt(var995)} as economic capital is incorrect. "
                                f"Economic capital = VaR − EL = {fmt(var995)} − {fmt(exp_loss)} = {fmt(eco_cap)}. "
                                f"Expected loss is reserved for; EC covers unexpected loss only.")
                       }
