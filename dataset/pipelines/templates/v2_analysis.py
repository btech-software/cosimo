...
def eq_fcff_dcf(rng, seq):
    ...
    answer = (f"NOPAT = {fmt(rev*o_margin*(1-tax))}M. Capex = {fmt(rev*capex_p)}M. "
              ...
              f"\n\nMechanism. The metric is computed through a structured analytical framework that applies multiple quantitative methods in sequence. The explicit forecast period captures years where detailed assumptions about revenue growth, margin evolution, and capital expenditures are most reliable and defensible. Beyond that horizon, a perpetuity formula assumes the firm reaches a sustainable steady state. The terminal value typically dominates in present value terms, making the assumptions about the long-run growth rate and discount rate spread the single most impactful variables in any valuation model. How the discount rate and growth rate interact through the denominator (r minus g) creates an exponential amplification when the spread narrows to just a few percentage points.\n\nTrade-offs. The primary tension is between precision and robustness: more explicit forecast years reduce terminal value reliance but increase model complexity and the number of assumptions that must hold simultaneously in equilibrium. Revenue growth must be justified by identifiable competitive advantages—network effects, switching costs, economies of scale, or brand moats that create pricing power. Operating margins should be benchmarked against industry peers and stress-tested against historical ranges under recessionary scenarios. Capital allocation decisions directly affect free cash flow generation through the capex-to-revenue ratio and working capital efficiency. The choice of discount rate blends cost of equity, target capital structure, and market risk premium assumptions.\n\nPerturbation. Sensitivity analysis quantifies how the metric moves with parameter changes in a scenario-weighted framework. A realistic approach uses base case weights (roughly 60 percent probability), bull case (roughly 20 percent), and bear case (roughly 20 percent). The resulting range is far more informative than any single point estimate and captures model uncertainty. Revenue growth assumptions should be calibrated against historical growth rates for comparable companies and industry growth forecasts from multiple sources. Key sensitivities include the discount rate, long-term growth assumption, margin trajectory, capital intensity, and working capital dynamics. A 25 basis point change in the discount rate can meaningfully shift the value, as can a 50 basis point shift in the terminal growth rate or a 100 basis point change in operating margins.\n\nContext. In practice at a quantitative investment firm, valuation models serve as analytical anchors for screening rather than precise price targets. Cross-validation is essential: if model-implied multiples differ by more than two times from comparable medians, this flags either mispricing or structural differences that require investigation. The output should be combined with relative valuation via comparable companies and precedent transactions to form a comprehensive price range. Analysts typically triangulate across multiple approaches to establish a trading multiple band and check for outliers. The valuation is ultimately a range, not a point estimate, reflecting inherent uncertainty in forward-looking assumptions about future growth, margins, and capital requirements.\n\nImplication. The computed result should be evaluated against multiple benchmarks: industry median multiples, historical trading ranges over the past three to five years, and implied growth rates from the current market price. Deviations from the peer median typically signal either a quality premium for superior execution, an expected margin improvement or deterioration, or market mispricing relative to fundamentals. Identifying which of these three explanations applies is the core skill that separates fundamental analysis from mechanical valuation. The analyst must also consider whether the gap reflects a transient factor or a structural advantage that could persist.\n\nBenchmarking. Comparing against sector-specific median multiples provides a useful reality check. If the computed value implies a multiple significantly above the sector median, the analyst must articulate the justification—superior growth profile, margin leadership, franchise value, or a structural cost advantage. If the implied multiple is below the median, the question is why the market is underestimating the business: deteriorating fundamentals, governance concerns, or market inefficiency that creates an opportunity. Historical multiples over full market cycles provide a range within which the current valuation should reasonably fall assuming normal economic conditions.")
...
def qs_time_series(rng, seq):
    ...
    stem = (f"50+ monthly returns: mean {pct(mean_ret)} per period, "
